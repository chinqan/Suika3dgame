...
> [!WARNING]
> CSS2DRenderer 使用 `transform: translate(...)` 定位子元素，因此浮動文字的 CSS 動畫**不可使用 `transform`**（包括 `translateY`、`scale` 等），否則會覆蓋定位導致文字跑到 (0,0)。應改用 `margin-top`、`opacity`、`font-size` 等不衝突的屬性。
...
| 區域 | 位置 | 說明 |
|------|------|------|
| SCORE | 容器正上方左側 | `#hud-top` flex 左對齊 |
| NEXT 3D 預覽 | 容器正上方右側 | 獨立 WebGLRenderer 80×80 |
| 排行榜/設定 | 畫面左側垂直排列 | `#hud-left-bar`, `position: absolute; left: 16px` |
| FPS 計數器 | 左下角 | `position: fixed; bottom: 8px; left: 8px` |
